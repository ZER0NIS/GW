...
{
	__super::Init();
	rade_db::IDatabase* p_DB = m_pWorld->GetDBAddr(DB_NONE);
	if (NULL != p_DB)
	{
...
void CConsortiaMgr::DealWith_DelConsortia(rade_db::PSQL_RESULT result)
{
	//记得改写存储过程，还回公会的ID号
	static int i = 0;
...
void CConsortiaMgr::DelConsortiaFormDB(ConsortiaElem* pConsrotiaInfo)
{
	...
	rade_db::IDatabase* DB = m_pWorld->GetDBAddr(DB_NONE);
	if (!DB)
	{
		ASSERT(0);
		return;
