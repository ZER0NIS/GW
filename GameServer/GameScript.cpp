...
void ExcScript(CGameObject* p1, CGameObject* p2, int MagicID)
{
	...
	pyScriptObject* pyo2 = NULL;
	if (p2) {
		pyo2 = (pyScriptObject*)PyObject_NEW(pyScriptObject, &pyPlayer_type);
		if (!pyo2) {
			Py_DECREF(pyo1);
			PyErr_Print();
			PyErr_Clear();
	...
	PyObject* pFunc = PyObject_GetAttrString(module, data->ScriptFun);
	if (!pFunc) {
		Py_DECREF(pyo1);
		if (pyo2) Py_DECREF(pyo2);
		PyErr_Print();
		PyErr_Clear();
		return;
	}
...
bool InitGameScript()
{
	...
	if (skill) {
		...
		printf("[ERROR] No se pudo importar Script.Skill\n");
	}
	...
	if (Altar) {
		...
		printf("[ERROR] No se pudo importar Script.Altar\n");
	}
	...
	if (SysFun) {
		...
		printf("[ERROR] No se pudo importar Script.SysFun\n");
	}
...
void Learn(PyObject*, PyObject* args)
{
	...
	if (!args || !PyTuple_Check(args) || PyTuple_Size(args) < 3) {
		printf("[ERROR] Learn: Argumentos inv醠idos\n");
		return;
	}
	...
	if (!arg0 || !arg2) {
		printf("[ERROR] Learn: No se pudieron obtener los argumentos\n");
		return;
	}
	...
	if (!obj1) {
		printf("[ERROR] Learn: ID de objeto inv醠ido\n");
		return;
	}
...
void AddHP(PyObject*, PyObject* args)
{
	...
	mgicPerform.uiObjectID = targetobj->GetID();
	mgicPerform.ucMagicID = magic;
