...
rade_db::IDatabase* CWorld::GetDBAddr(DB_TYPE dbType)
{
	rade_db::IDatabase* pDb = NULL;
...
void CWorld::InitDataCore(void)
{
	...
	for (int i = 0, j = 1; i < g_lMainCity; i++, j++)
	{
		g_pRegion[i].SetID(i);
	...
	g_lAllScene = g_lMainCity + g_lCity + g_lField;
...
void CWorld::SetPlayer(CPlayer* pPlayer, bool bState)
{
	MAP_PLAYER::iterator itor = g_setPlayer.find(pPlayer);
