...
namespace sbase
{
	...
	public: // get
		int		GetEscape() const { return m_tUpdateTime ? __max(((int)::time(NULL) - (int)m_tUpdateTime), 0) : 0; }
		void	StartDayCheck() { Startup(GetRemainSecsToZero()); }
		bool	IsDayPassed() { if (IsActive() && IsExpire()) return Startup(GetRemainSecsToZero()), true; return false; }
		DWORD	GetRemainSecsToZero();
		DWORD	GetPassedSecs() { if (IsActive()) return (DWORD)(::time(NULL) - m_tUpdateTime); return 0; }
		void	StartHourCheck() { Startup(GetRemainSecsToHour()); }
		bool	IsHourPassed() { if (IsActive() && IsExpire()) return Startup(GetRemainSecsToHour()), true; return false; }
		DWORD	GetRemainSecsToHour();
	protected:
		int		m_nInterval;
