...
namespace rade_db
{
	...
	protected:
		CRecordset(CDatabase& rade_db, MODE eMode);
		virtual ~CRecordset();
	private:
		CDatabase& m_db;
		MODE			m_eMode;
	...
	public:
		...
		virtual unsigned long	CountRecord(void) const { return (unsigned int)m_setRecord.size(); }
	...
	protected:
		...
		int					FieldsCount(void) const { return (unsigned int)m_setFieldInfo.size(); }
